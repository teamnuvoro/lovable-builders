...
export const createDailyRoom = async () => {
  const response = await fetch('https://api.daily.co/v1/rooms', {
    ...
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${CallingConfig.daily.apiKey}`
    },
    body: JSON.stringify({
      properties: {
        exp: Math.floor(Date.now() / 1000) + CallingConfig.settings.maxCallDuration,
        enable_screenshare: CallingConfig.settings.enableScreenShare,
        enable_recording: CallingConfig.settings.enableRecording ? 'cloud' : 'off',
        start_video_off: !CallingConfig.settings.autoStartVideo,
        start_audio_off: !CallingConfig.settings.autoStartAudio
      }
    })
  });
